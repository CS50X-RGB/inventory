...
export default function PlanningHistoryViewAll() {
    ...
    const { data: getBom, isFetching, isFetched } = useQuery({
        ...
        queryFn: async () => {
            return getData(`${bomRoutes.getTransaction}/${page}/5?status=${status}`, {});
        }
    });
    useEffect(() => {
        if (isFetched) {
    ...
    return (
        <div className="flex flex-col gap-4 p-8">
            ...
            <Select onSelectionChange={(e) => {
                const value : any = Array.from(e)[0]; // extract value from Set
                setStatus(value);
